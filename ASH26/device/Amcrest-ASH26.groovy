...
def hash(algorithm, text) {
    return new String(java.security.MessageDigest.getInstance(algorithm).digest(text.bytes).collect { String.format("%02x", it) }.join(''))
}
            ...
            if (e1.response.status == 401) {
                ...
                try {
                    httpGet(params) { resp ->
                        ...
                        resp.data.text.splitEachLine("=") { items ->
                            if (items.size() == 2) {
                    ...
                    log.debug("get_camera_settings failed: " + e2.response.status)
                }
            }
        }            
	}
            ...
            try {
                httpGet(params) { resp ->
                    if (resp.data.text.trim() != ("OK")) {
                        log.debug("set_camera_setting: error setting " + setting + ": " + resp.data.text)
                    } else {
                        log.debug("set_camera_setting: " + setting + "=" + value)
                    }
                }    
            }
            catch (groovyx.net.http.HttpResponseException e2) {
                log.debug("set_camera_setting failed: " + e2.response.status)
            }
        }
    }
}
...
def initialize() {
    ...
    if (!ip || !username || !password) {
        log.warn "Amcrest ASH26 required fields not completed.  Please complete for proper operation."
        return
    }
